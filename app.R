...
ui <- fluidPage(theme = shinytheme("cerulean"),
                ...
                navbarPage(
                    ...
                    tabPanel("5. Analyze data",
                            ...
                            numericInput(inputId = "clusters", label = "Clusters", value = 1, min = 1, max = 10,step = 0.5, width = "80px"),
  ...
  sets = eventReactive(input$some, {
    ...
    output$xlsxdownload <- downloadHandler(
      ...
      content = function(file) {
        wb = createWorkbook()
        admin = data.table::fread(input$admindf$datapath, data.table = FALSE)
        ind_betas = data.table::fread("RBetas.csv", data.table = FALSE) # is created as part of function cust_choicemodelr
        analysis_df = analysis()[[5]]
        
        cluster_colnames = c()

        for(attri in 1:nrow(analysis_df)){
          colname_element = paste(analysis_df$all_attributes[[attri]],analysis_df$all_levels[[attri]], sep ="_")
          cluster_colnames = append(cluster_colnames,colname_element)
        }
        
        
        if (analysis_df$all_attributes[nrow(analysis_df)] == "None") {
          cluster_colnames[length(cluster_colnames)] = "None"
        }
        
        colnames(ind_betas) <- cluster_colnames
        
        
        if (input$clusters > 1){
          kcluster <- kmeans(ind_betas, input$clusters)
          utility_cluster <- kcluster$cluster
          ind_betas <- cbind(ind_betas,utility_cluster)
          last_col_index <- ncol(ind_betas)
          aggregated_df = aggregate(ind_betas[, 1:(last_col_index-1)], by = list(ind_betas[, last_col_index]), mean)
          names(aggregated_df)[names(aggregated_df) == "Group.1"] <- "Cluster"
          # aggregated_df[, -1] <- scale(aggregated_df[, -1]) # option to make differences among clusters more clear. might be misleading tho
          
          mean_df_long <- reshape2::melt(aggregated_df, id.vars = "Cluster", variable.name = "Level", value.name = "Mean")
          
          # create heatmap
          plotted_means = ggplot(mean_df_long, aes(x = Cluster, y = Level, fill = Mean)) +
            geom_tile() +
            geom_text(aes(label=round(Mean,2))) +
            scale_fill_gradient(low = "blue", high = "yellow") +
            labs(x = "Cluster", y = "Level", title = "Mean partworth utilities per cluster")
          ggsave("graph.png", plot = plotted_means)
          
        }

        admin = cbind(admin, ind_betas)
        addWorksheet(wb, "data")
        writeData(wb,"data", admin)
        num_rows_rawdata <- nrow(admin)
        num_cols_rawdata <- ncol(admin)
        
        if (input$clusters > 1){
          writeData(wb, "data", aggregated_df, startRow = 1, startCol = num_cols_rawdata + 2)
          num_rows_aggregated_df <- nrow(aggregated_df)
          insertImage(wb, "data", "graph.png", width = nrow(aggregated_df), height = nrow(aggregated_df), startRow = num_rows_aggregated_df + 5, startCol = num_cols_rawdata + 2)
        }
        
        addWorksheet(wb, "all_products")
        writeData(wb,"all_products", analysis()[[3]])
